...
## #21 — 2026-09-09 — הסנטינל האדום מדד את הגל של עצמו: `\b` מול תג-הגיבוי של הלוגו

אבחון read-only (§8.1 שלב 1) בעקבות התראת דיסקורד **2026-09-09T18:02Z**. ⛔ **0 קבצי קוד ·
0 `tests-sentinel/` · 0 workflows · 0 DB · 0 ספים · 0 השתקות.** שתי השערות הוצבו **מראש** בפרומפט:
**(א)** יצירת עסקה שבורה בפרודקשן (`P0`) · **(ב)** הגל של היום שבר את הספק. 🔴 **(ב) אוששה
במדידה · (א) הופרכה בראיה ישירה.**

- 🔴 **(א) הופרכה — העסקה **כן** נוצרה בפרודקשן.** לוג הריצה `34386467496` נושא
  `sentinel rest cleanup: removed 1 SNTNL/SNTNL1 row(s)` ב-`18:02:11Z`: שורה בטיקר הבדיקה
  **הגיעה ל-`public.trades`** דרך הטופס בדפדפן אמיתי עם חשבון מחובר, ונקראה חזרה ב-REST.
  ⇒ מסלול הכתיבה **חי**, ו**⛔ אין משתמשים מושפעים.** ⚠️ **הראיה היא ה*ניקוי*, ⛔ האסרציה** —
  האסרציה היא בדיוק מה שנשבר.
- 🔴 **הסיבה, נמדדה ⛔ ולא הוסקה: `TickerLogo` מרנדר **טקסט** כשהלוגו נופל, והטקסט מודבק לטיקר.**
  `src/components/TickerLogo.jsx:8-14` — `onError` ⇒ `imgError` ⇒ `<div>{cleanTicker.slice(0,2)}</div>`.
  ב-`SwingEdge_App.jsx:5335` היושבים `<TickerLogo …/>{t.ticker}` **צמודים** ⇒ ⛔ אין צומת-טקסט ביניהם.
  נמדד ב-Chromium אמיתי מול ה-DOM המדויק: `textContent` של השורה הוא **`"SNSNTNL …"`**.
  ⇒ התו שלפני `SNTNL` הוא `N` (תו-מילה) ⇒ **`\b` ⛔ קיים** ⇒ `0` התאמות.
- **המנייה המלאה (מדידה, `/tmp`, ⛔ בריפו):** `/\bSNTNL\b/` → **`0`** · `/\bSNTNL1\b/` → **`0`** ·
  תת-מחרוזת `'SNTNL'` (הנוסח הישן) → **`2`** · `'SNTNL1'` → **`1`**. 🔴 **זרוע בקרה:**
  `/\bAAPL\b/` → **`1`**, כי הלוגו האמיתי **נטען** ⇒ `<img>` ו-`alt` ⛔ נכנס ל-`textContent`.
  ⇒ **המבדיל הוא תג-הגיבוי, ⛔ הרגקס לבדו.** מקור ההבדל נמדד ישירות:
  `financialmodelingprep.com/image-stock/` מחזיר **`404` ל-`SNTNL`** · **`404` ל-`SNTNL1`** ·
  **`200` ל-`AAPL`** ⇒ **`2/2` טיקרי הבדיקה נופלים תמיד** ו-`1/1` הטיקר האמיתי ⛔ נופל לעולם.
  🔴 **⇒ הפגם ⛔ יכול היה להיראות על שלוש העסקאות הקדושות** — הן ⛔ נבדקות ב-`\b`.
- 🔴 **הגל שבר את עצמו, וההערה שלו טוענת מדידה ש⛔ נעשתה.** `744f26b` (`B-305`, 19:48 IL) החליף
  `hasText: TICKER` (תת-מחרוזת) ב-`hasText: /\bSNTNL\b/`. ההערה בקובץ
  (`sentinel-auth.spec.js:252-256`) כותבת מפורשות ש-`\b` «דוחה `SNTNL1` ומקבל `SNTNL`» ו-
  **«Verified mutually exclusive»**. נמדד: הוא דוחה את **שניהם**. ⚠️ **הבדיקה שנטענה בהערה נעשתה
  על המחרוזת, ⛔ על ה-DOM** — טענת-אימות בהערה ⛔ **אינה** אימות, וזו `R-4` בתחפושת הערה.
- **`B-305` נסגר ב-`9f3c397` (19:50 IL) כ«אומת-מקור», והריצה האמיתית הראשונה שלו נפלה `71` דקות
  אחר כך.** ⚠️ **וזה ⛔ כשל של `probe:firing`** — הוא מדד **ערך** (`riskPct=null` ⇒ throw) ונשאר
  נכון; `D-077` **הצהיר במפורש** ש«הספק ⛔ הורץ מול Supabase אמיתי · ⛔ דפדפן · ⛔ התחברות».
  🔴 **ההצהרה הייתה נכונה — והפער שהיא תיארה הוא בדיוק מה שנפל.** ⇒ הלקח ⛔ «להצהיר טוב יותר»,
  אלא ש**גבול מוצהר ⛔ אינו גבול מכוסה**.
- **`B-304` זוכה בראיה, ⛔ בהיסק.** בריצה האדומה נספרו **`2`** ממצאים בסך הכל: `1` אדום
  (`browser-auth|create-failed`) ו-`1` צהוב (`boundary-unmeasurable|mentoring`, צפוי — הטאב נעדר
  בחשבון בלי חניכים). ⇒ **`0` `panel-crashed` מתוך `5` פאנלים · `0` `root-crashed`** ⇒ ⛔ אף גבול
  ⛔ נפתח, והג'ורנל **רונדר**. ⚠️ **וההיסטוריה לבדה ⛔ יכלה להכריע:** הריצה הירוקה שלפני
  (`34364955931`, 14:38Z, אחרי גל הגבולות) נשאה **`SENTINEL_AUTH: 0`** ⇒ שכבת המחובר **דולגה**
  ⇒ היא ⛔ ראיה. הריצה האחרונה עם `AUTH=1` לפני השבירה היא **09:50Z על `763137f`** — **לפני**
  גל הגבולות ⇒ בין הירוק לאדום זזו **`2`** דברים, וההכרעה באה מהריצה האדומה עצמה, ⛔ מהרצף.
- 🔴 **רדיוס הפגיעה גדול מהאסרציה שצעקה — `4/4` הלוקייטורים של טיקרי הבדיקה מתים:**
  `rowsFor` · `sntnlRows` · `testRows` · `deleteRow` — כולם נגזרים מאותו `\b`. ⇒ **סריקת השרידים
  (`stale-testdata`) ⛔ יכולה לירות** ו-`deleteRow` ⛔ יכול למחוק ב-UI. ⚠️ **מה שמונע הצטברות
  שורות בדיקה בפרודקשן הוא `restCleanup` בלבד** — הוא מסנן `ticker=in.(SNTNL,SNTNL1)` ב-DB
  ו⛔ ברגקס. 🔴 **⇒ מסלול ה-`cleanup-unconfigured` (סודות Supabase חסרים) הפך מ«רשת ביטחון» ל**נקודת
  הכשל היחידה**: בלעדיו כל ריצה שעתית משאירה שורה יתומה בפרודקשן.**
- **מה ⛔ נמדד, מוצהר ו⛔ הוחלף בניחוש:** מדידה חוזרת של `M-004` (עסקאות ב-48ש׳) — ה-`.env`
  המקומי נושא `VITE_SUPABASE_URL` ו-`VITE_SUPABASE_ANON_KEY` באורך **`0` תווים** ⇒ ⛔ אין גישה,
  ו-RLS חוסם ממילא קריאת עסקאות של אחרים (§12 — סודות הם של ניב). ⚠️ **הערך `M-004` מ-11:21Z
  היום נשאר בתוקף ו⛔ עודכן.** ⛔ **וגם ⛔ נבדק** האם `SNTNL1` היה מייצר את קריסת 07.09 בפועל —
  המסע נעצר לפני שלב `5b`, ⇒ **`0/5`** אסרציות הגבולות ו-**`0/1`** שינוי-השם **⛔ נצפו רצים מול
  DOM אמיתי**; `C-044` נשאר פתוח במלואו.
- **הלקח, וההכללה שלו ⛔ «לתקן רגקס»:** *שער שנכתב מול ה**מחרוזת** שהמפתח מדמיין, ⛔ מול ה**בייטים**
  שהדפדפן מרנדר, מודד את הדמיון.* זו אותה משפחה של «CSS סטטי ⛔ קובע מה מרונדר» (`test:analytics`)
  ושל «`S1`–`S4` קוראות בייטים» (`INCIDENTS#20` נספח ב׳) — ⛔ **כיסוי חסר, אלא סוג-ראיה שגוי.**
  ⚠️ **ותוצאת-לוואי שאסור לפספס: זו הפעם הראשונה שהסנטינל התריע על עצמו תוך שעה** — ⛔ בזכות
  שכבה חדשה, אלא כי הפעם הפגם ישב במסלול ש**כן** נדגם. ⛔ **⇒ אין להסיק ש-`B-275`/`B-306` נסגרו.**

### #21 · נספח מתקן, 2026-09-09 — **גבול שהוצהר ⛔ גבול שכוסה**

הגוף למעלה קבע את הלקח («שער שנכתב מול המחרוזת שהמפתח מדמיין מודד את הדמיון»)
וסיים ב«גבול מוצהר ⛔ אינו גבול מכוסה». הנספח הזה הופך את המשפט הזה מ**תיאור**
ל**כלל**, כי בלעדיו הוא ⛔ מונע את החזרה.

- 🔴 **הכלל:** **`אומת-מקור` ⛔ יכול לסגור פריט שכל מהותו היא ריצת דפדפן.**
  `CLAUDE.md` §15 מתיר `אומת-מקור` ל-«`T3` **בלי מסך מושפע**» — ו«בלי מסך מושפע»
  נמדד לפי **מה שהפריט משנה**, ⛔ לפי מה שהגל **נגע** בו. `B-305` ⛔ נגע ב-`src/`
  (`0` קבצי מוצר, נכון ומדויק) — אבל הוא שינה `4/4` לוקייטורים ש**כל קיומם**
  הוא ריצה בדפדפן. ⇒ היה לו מסך מושפע, וההיתר ⛔ חל עליו.
- **מה שהיה ⛔ מספיק, למרות שנראה כך.** `D-077` ⛔ הסתיר דבר: הוא כתב במפורש
  «⛔ **הספק ⛔ הורץ מול Supabase אמיתי** — ⛔ דפדפן · ⛔ התחברות · ⛔ `SNTNL1`
  נוצר בפרודקשן». 🔴 **ההצהרה הייתה נכונה, מלאה, ובלטה — והריצה נפלה `71` דקות
  אחר כך בדיוק בפער שהיא תיארה.** ⇒ **הצהרת-פער מתעדת סגירה ו⛔ מסמיכה אותה.**
  הלקח ⛔ «להצהיר טוב יותר»; הוא **שהצהרה ⛔ מחליפה ראיה**.
- ⚠️ **וזו ⛔ ביקורת על `probe:firing`.** הוא מדד **ערך** — `riskPct === null` ⇒
  `51a12d2` זורק · העץ הנוכחי מרנדר `"—"` — והוא **נשאר נכון עד היום**; `B-312`
  ⛔ הפריך ממנו אף אסרציה. הפגם הוא בהתאמה בין **סוג-הראיה** ל**סוג-הפריט**:
  ראיית-ערך על פריט שמהותו לוקייטור היא ראיה **על שכבה אחרת**.
- **מה זז בפועל (09.09, `B-312`):** `4/4` הלוקייטורים עברו לעוגן-תכונה
  `[data-testid="trade-ticker-<sym>"]` — שוויון-תכונה הוא בלעדיות **בבנייה**,
  ⛔ בטענה. ⛔ **ו-`B-312` ⛔ נסגר**: מבחן הירי מול פרודקשן הוא **⛔ נצפה — חסום**
  (`6/6` סודות QA באורך `0`), ו-`C-044` פתוח במלואו.
- 🆕 **09.09 20:11Z — החסם הוסר ו-`B-312` עדיין ⛔ נסגר, מסיבה אחרת.** ריצה
  `34399564706` רצה עם `SENTINEL_AUTH: 1` וסודות QA **חיים**; המסע רץ **במלואו**
  (יצירה · שינוי-שם · סריקת `5` גבולות · מחיקה) ו-`sentinel rest cleanup: removed 0`.
  🔴 **ובכל זאת ⛔ ניתן לצטט את מדד הסגירה** — ל-spec `2` `console.log` בלבד, ו-`3`
  הבקרות הן אסרציות **שותקות בהצלחה**. ⇒ **הלקח החוזר, שכבר שלישי בשרשרת הזו:**
  `D-077` נסגר על **הצהרה** במקום ראיה; `B-312` נולד כי **הערה** טענה אימות; וכעת
  מדד-הסגירה עצמו נכתב כ«נצפו» מול מערכת ש⛔ **מדפיסה דבר** ⇒ ראיה שקיימת
  ב**היעדר-ממצא** בלבד ⛔ ניתנת לציטוט, והיא נשענה כאן על ממצא צהוב **מקרי**
  (`boundary-unmeasurable|mentoring`) שנשא את מחרוזת ה-`phase`. ⚠️ **ריצה ירוקה
  לגמרי הייתה מותירה `findings: 0` ו⛔ כלום.** ⇒ **מדד סגירה חייב לנקוב בפלט
  שהמערכת באמת פולטת** — אחרת הוא ⛔ מדד, הוא משאלה.
- ⛔ **`D-077` ⛔ נמחק** (§14 — מזהה לא נמחק ולא ממוחזר). הוא נושא שורת תיקון
  מתוארכת ב-`DONE.md`, ו-`DECISIONS` 09.09 נושא את ההכרעה.

---

## #22 — 2026-09-10 — מודל העריכה נראה פריים אחד לפני שהוא מלא, והמנייה של `B-312` יושבת אחרי הגדר שנפלה

אבחון read-only (§8.1 שלב 1) על ריצה `34467180681` (`#885` · 10:39Z · `workflow_dispatch` ·
`SENTINEL_AUTH: 1` · `Success` · `2m 22s` · commit `808d83d`). ⛔ **0 קבצי קוד · 0 `src/` ·
0 `api/` · 0 `tests-sentinel/` · 0 workflows · 0 DB · 0 ספים · 0 השתקות · 0 ריצות חוזרות.**
ההנחה שבפרומפט — «`logAnchorCounts` ⛔ רץ» — **אוששה במדידה**; השערת השורה היתומה
**הופרכה בשלוש ראיות בלתי-תלויות**.

- 🔴 **ההנחה אוששה.** `sentinel anchor:` · `sentinel control:` · `unmeasured` — **0 מופעים**
  בלוג הריצה. ⛔ **והקוד היה בעץ:** `git diff a3cb43a 808d83d -- tests-sentinel/sentinel-auth.spec.js`
  מוסיף את `ANCHOR_CONTROL`, את `logAnchorCounts` ואת אתר-הקריאה היחיד — **`+39` שורות,
  `0` מחיקות, `0` שינויים**. ⇒ ⛔ קוד חסר, ⛔ באנדל ישן — **מסלול שלא נגעו בו**.
- 🔴 **8 הממצאים, ו-`7/8` מהם הם באג אחד.** `1` 🔴 `fixture-failed` · `5` 🟡
  `boundary-unmeasurable` (`risk`·`journal`·`mentoring`·`analytics`·`watchlist`) · `1` 🔴
  `tab-switch` · `1` 🔴 `delete-failed`. ⚠️ **`boundary-unmeasurable|mentoring` הוא היחיד
  שאינו נגזרת** — הוא הממצא **היחיד** של `#880` אתמול, ומקורו בחשבון QA בלי חניכים.
- 🔴 **למה `logAnchorCounts` ⛔ רץ — מדוד, ⛔ מוסק.** היא ה**משפט האחרון** של
  `renameToRiskTicker` (`:476`), אחרי ארבעה משפטים שיכולים לזרוק. הזורק שירה הוא הגדר
  ב-`:466-468`, וה-`got` של הממצא זהה **בייט-בבייט** לתבנית שם:
  `edit modal guard failed: first input holds "", expected SNTNL`. הזריקה נתפסת ב-`:767`
  ⇒ `fixtureLive` נשאר `false` ⇒ `:476` ⛔ נקרא. ⚠️ **וכל שער-יציאה מוקדם יותר ⛔ ירה** —
  `secrets-missing` · `login-failed` · `hydration-timeout` · `journal-open` · `create-failed`
  **נעדרים כולם** מ-8 הממצאים ⇒ המסע הגיע ל-`:465` ונפל שם בדיוק.
- 🔴 **והפגם המבני: המנייה יושבת *אחרי* האסרציות (`:474-475`), ⛔ לפניהן.** ⇒ היא מדפיסה
  **רק בריצה שכבר הצליחה במלואה**. `B-312` נולד כי «אסרציה עוברת ⛔ מדפיסה דבר»; התיקון
  הזיז את העיוורון מקום אחד — מ«ירוק ⛔ מדפיס» ל«מדפיס רק כשירוק». ⚠️ **בדיוק בריצה שבה
  המספר היה שווה משהו, הוא ⛔ הודפס.** זו הפעם ה**רביעית** בשרשרת הזו (`D-077` נסגר על
  הצהרה · `B-312` נולד מהערה שטענה אימות · מדד-הסגירה נכתב מול מערכת שותקת · וכעת המדפיס
  עצמו מותנה בהצלחה).
- 🔴 **שורש הכשל בצד המוצר — מרוץ-רינדור, מדוד בחמישה שלבים:** ⓵ `<EditTradeModal>`
  מרונדר **ללא תנאי** (`SwingEdge_App.jsx:8266`) — ⛔ `{editingTrade && …}` · ⓶
  `useState(() => initForm(trade))` (`:23`) הוא אתחול-עצל שרץ **פעם אחת**, בעליית האפליקציה,
  כש-`editingTrade === null` · ⓷ `initForm(null)` (`:420-432`) מחזיר `ticker: ""` — **בדיוק
  ה-`""` שהגדר קראה** · ⓸ `if (!trade) return null` (`:41`) מחזיק אותו מחוץ ל-DOM עד לחיצת
  «עריכה»; ברינדור הזה `trade` כבר מלא אבל `form` עדיין נגזר-מ-`null` ⇒ **הדיאלוג נצבע עם
  `value=""`** · ⓹ ה-`useEffect` (`:27-30`, מפתח `[trade?.id]`) רץ **אחרי** הצביעה ומקפיץ
  `setForm` ⇒ רינדור שני עם `SNTNL`. ⇒ **קיים פריים אחד בדיוק שבו `[role="dialog"]` גלוי
  ושדה הטיקר ריק**, ו-`dialog.waitFor({state:'visible'})` ב-`:463` יכול להיפתר עליו.
- ⚠️ **והראיה ⛔ תיאורטית — היא באותה ריצה.** ה-call-log של `boundary-unmeasurable|analytics`
  נושא את האלמנט החוסם `<input value="SNTNL" class="w-full bg-white/5 … font-mono font-bold"/>`,
  ומחרוזת ה-class זהה **בייט-בבייט** ל-`EditTradeModal.jsx:151`. ⇒ **אותו input, אותה ריצה:
  `""` ב-`:465`, `SNTNL` רגעים אחר כך.** ⛔ **ואין כאן טענה על נראות למשתמש** — פריים אחד;
  הנזק ה**מדוד** הוא לסנטינל בלבד.
- 🔴 **ולמה זה הפיל את כל השאר: הזריקה השאירה את המודל פתוח.** ⛔ אין ענף שסוגר אותו במסלול
  הזה ⇒ `<div class="fixed inset-0 z-50 …">` חוסם pointer-events, וכל לחיצה שאחריה נופלת
  ב-`Timeout 15000ms` — `5` טאבי הגבולות · החזרה ליומן · כפתור המחיקה. **באג אחד, `7`
  ממצאים.**
- 🔴 **איזו שורה נשארה — `SNTNL`, ⛔ `SNTNL1`.** `fixtureLive === false` ⇒ `5/5` מחרוזות
  ה-`checked` של הגבולות נושאות «⚠️ `SNTNL1` ⛔ ביומן»; `deleteRow` נקראה עם `TICKER`,
  והלוקייטור בממצא הוא `[data-testid="trade-ticker-SNTNL"]`. ⚠️ **והיא עברה את שני
  השומרים** (`:437` `n === 1` · `:443` `cells === 1`) — הכשל בא מה**לחיצה** ב-`:444`
  ⇒ **בדיוק שורה אחת של `SNTNL` הייתה קיימת**.
- 🔴 **השערת השורה היתומה — הופרכה, בשלוש ראיות בלתי-תלויות.** ⓵ `browser-auth|stale-testdata`
  (`:656-658`, יורה אם `testRows().count() > 0` בפתיחת היומן) — **נעדר** ⇒ `0` שאריות בכניסה ·
  ⓶ הגדר `expected exactly 1 SNTNL row to rename, found N` (`:460`) — **⛔ ירה** ·
  ⓷ `#880` רשמה `removed 0` ⇒ ⛔ הועבר דבר קדימה. ⇒ **כל שומרי «בדיוק 1» ראו בדיוק 1;
  שורה יתומה ⛔ גרמה לכשל הזה.**
- 🔴 **⛔ אין שורה יתומה בפרודקשן עכשיו.** המסע נפל **אחרי** היצירה (`created === true`),
  והמחיקה ב-UI נכשלה ⇒ השורה שרדה את המבחן. רשת ה-REST ב-`afterAll` סחפה אותה:
  `sentinel rest cleanup: removed 1 SNTNL/SNTNL1 row(s)` ב-`10:41:15Z`, על מסנן
  `ticker=in.(SNTNL,SNTNL1)` עם `Prefer: return=representation` — DELETE מוחק **את כל**
  התואמות והחזיר **בדיוק 1** ⇒ `0` נשארו. ומאז רצה `#886` בלבד, עם `SENTINEL_AUTH: 0`
  ⇒ `1 skipped`, ⛔ נוצר דבר, ו-`afterAll` חוזר מוקדם. ⚠️ **ה-RLS מגביל את הסחיפה לחשבון
  ה-QA — החשבון היחיד שה-spec כותב אליו.** ⚠️ **`ui-delete-incomplete` ⛔ ירה, ובצדק** —
  שערו הוא `n > 0 && uiDeleteOk`, ו-`uiDeleteOk` היה `false`.
- 🔴 **מה השתנה בין `#880` ל-`#885` מלבד הקומיט — ⛔ כלום, וזו הנקודה.** הדיף על ה-spec הוא
  **תוספתי בלבד**, ו-`git diff --stat a3cb43a 808d83d -- src/ api/` **ריק**. ⇒ קוד המסע
  ובייטי המוצר **זהים** בשתי הריצות, וההפרש `1 → 8` הוא **תזמון** — שזו הגדרת מרוץ:
  `#880` דגמה אחרי הרינדור השני, `#885` דגמה על הראשון. ⚠️ **ו-`#886` ⛔ זרוע השוואה** —
  `schedule` ב-`11:26Z` ⇒ `MIN=26` ⇒ `auth=0` לפי שער ה-workflow (`sentinel.yml:79-87`).
- 🔴 **הלקח:** שומר שקורא **ערך** תוך-כדי שהוא ממתין ל**נראוּת** מודד את **לוח-הזמנים של
  הרינדור**, ⛔ את המוצר — והוא ייפול אקראית לנצח. ⚠️ **וזו ⛔ ביקורת על עצם הגדר** —
  `:466-468` היא בדיוק «מסרב אלא אם ודאי», והיא **תפסה פגם אמיתי בצד המוצר** שאף שער אחר
  ⛔ רואה. הפגם הוא ב**ערוץ ההמתנה**, ⛔ בשאלה.
- ⚠️ **§10.1 — הממצא של `EditTradeModal` ⛔ נושא `B-` עדיין.** `docs/STATE.md` נמדד
  `15,991/16,000` בתים (`9` פנויים), ורישום ב-`BACKLOG` מחייב את חשבונאות §8.6 (ארבעה
  מספרים בפרוזת שכבת השורשים). **הפריט ממתין להכרעת ניב** ⛔ ולא נרשם בניחוש.

---

## #23 — 2026-09-11 — שלוש התראות סנטינל, שורש אחד: HTML מטמון שהצביע על chunk שנגזם

**רישום בלבד.** ⛔ **0 קבצי קוד · 0 `src/` · 0 `api/` · 0 workflows · 0 DB · 0 תיקון ·
0 חקירה** — הפרומפט אוסר את שלושת האחרונים במפורש. שלוש התראות נחתו ב-**11.09 14:38Z**.
⚠️ **ארבע המדידות שלמטה נלקחו מפרודקשן בידי ניב ונמסרו בפרומפט — הגל הזה ⛔ מדד אותן
מחדש**, ולכן הן נרשמות כ**מצוטטות**, ⛔ כ«נמדד על ידינו».

- 🔴 **הגרסה שנטענה תאמה את הראש, ובכל זאת חסר chunk.** `origin/main` = `d0d7940` ·
  `SENTRY_RELEASE.id` בבאנדל = `d0d7940`. ⛔ **ועם זאת:** `assets/recharts-DZljt4aM.js`
  ⇒ **`404`**, בעוד ה-chunk שפרודקשן מגישה הוא `recharts-B7pIFb4y.js`.
- 🔴 **`/app` ⇒ `200` — ה-HTML נמסר, ו⛔ נכשל.** הכותרות נושאות `max-age=0, must-revalidate`
  **וגם** `x-vercel-cache: HIT` ⇒ מטמון קצה **החזיר** מסמך שאמור היה להיות מאומת-מחדש.
- 🔴 **השורש:** הסנטינל טען **HTML מטמון** שהצביע על chunk מפריסה **קודמת**, ש-Vercel כבר
  גזם ⇒ `404` ⇒ המודול ⛔ נטען ⇒ `AuthScreen` ⛔ רונדר ⇒ האדום. ⇒ **`3/3` ההתראות = ממצא
  אחד** (⛔ שלוש תקלות).
- 🔴 **ההצהרה: חלון-מרוץ פריסה, ⛔ באג אפליקציה.** ⛔ **ו⛔ false-positive** — זה מצב
  **אמיתי** ברגע המדידה: משתמש אנושי שטוען את `/app` באותה שנייה מקבל בדיוק את אותו מסך
  ריק. **האתר תקין עכשיו.** ⚠️ **ההבחנה חשובה** — «חיצוני לקוד שלנו» ⛔ «לא קרה»; קריאת
  התראה כזו כרעש היא בדיוק איך שערים נהיים ירוקים על מוצר מת.
- 🔴 **הסנטינל תפס נכון — וזו הפעם הראשונה שהוא מוכיח זאת על אירוע ש⛔ בנינו לו.**
  `3/3` דווחו, ⛔ אחת נבלעה; `B-305` (הסנטינל **מדווח**) ו-`B-312` (המנייה) **עבדו**.
  ⚠️ **⛔ להסיק מכאן שפער-ההתראה נסגר** — `B-306`·`B-308`·`B-310` פתוחים, ו-`6/6` «אין
  issues חדשים» עוד יודפס. **אירוע בודד ⛔ מדד;** המדד הוא מדידה חוזרת של `M-010`.
- ⚠️ **§11 — תצפית שנרשמה בטיוטה ו*הופרכה באותו גל*, ⛔ שרדה לרשומה.** הטיוטה טענה
  ש-`npm run build` המקומי על `d0d7940` פלט `dist/assets/recharts-DZljt4aM.js` — השם
  שחזר `404` — ובנתה על כך כיוון. 🔴 **המדידה הישירה מפריכה:** `npm run verify` על העץ
  הנוכחי (`d0d7940` + שינויי `docs` **בלבד**, ⛔ קובץ קוד) פלט `dist/assets/recharts-B7pIFb4y.js`
  — **בדיוק ה-chunk שפרודקשן מגישה**. ⇒ **השם שנגזם ⛔ מיוצר ע"י העץ הזה**, וההשערה נרשמת
  כ**מופרכת** ⛔ כתצפית פתוחה. ⛔ **⛔ נחקר מעבר לכך** (הפרומפט אוסר חקירה). ⚠️ **והיא
  נרשמת ולא נמחקת** — הנחה שהוכחשה היא ראיה; מחיקתה הייתה משאירה את הכיוון השגוי זמין
  למי שיעלה אותו שוב.
- 🔴 **הפריט שנפתח: `B-320` — חלון מרוץ פריסה.** ⛔ **לא תוקן בכוונה.** הכיוונים שנמסרו
  (⛔ ביצוע): הגשת chunks ישנים לתקופת חסד · טיפול ב-`vite:preloadError` · באנר «גרסה
  חדשה — רענן». ⛔ **הגוף ב-`BACKLOG` ⛔ ולא כאן** (§14).

### 🔴 תיקון 2026-09-13 — הבולט «§11 · הופרכה» הוא **שיוך שגוי**

⛔ **הטקסט למעלה ⛔ נמחק** (הנחה שהוכחשה היא ראיה). הוא **מסומן כשגוי**:

- 🔴 **«הופרכה» הייתה בפועל *אישוש*.** ההשערה הייתה «ה-`404` הוא HTML מפריסה
  **קודמת**». הממצא «העץ הנוכחי מייצר שם **אחר** מזה שנגזם» הוא **בדיוק מה
  שההשערה חוזה** — לו העץ הנוכחי היה מייצר את `recharts-DZljt4aM.js`, ההשערה
  הייתה **מופרכת**. הראיה נקראה בסימן הפוך.
- 🔴 **והאישוש נמדד ישירות** — שמות ה-vendor המקומיים משחזרים את פרודקשן לפי קומיט:
  `d0d7940` ⇒ `recharts-B7pIFb4y.js` (= פרודקשן 17:2x) · `9b371a7` ⇒
  `recharts-B-SlmYtq.js` (= פרודקשן 17:4x), **`2/2` התאמות מדויקות**.
  ⇒ `recharts-DZljt4aM.js` שייך לקומיט **מוקדם יותר** — ⛔ ל-`d0d7940`.
  ⚠️ **סייג:** שמות ה-`index` ⛔ תואמים (`index` מטמיע `VITE_*` מה-`.env` המקומי,
  שערכיו ⛔ ערכי Vercel) ⇒ **chunks של vendor הם ההשוואה התקפה, `index` ⛔.**
- ⚠️ **ותיקון טופולוגיה — הניסוח «שרשרת» היה שגוי, המסקנה נכונה.** ⛔ **`recharts`
  ⛔ מייבא מ-`index`** — נמדד `import{…}from"./sentry-BxvQ_CH4.js"`. ⛔ **אין
  שרשרת:** `release.inject` (⛔ הושבת) מחתים `SENTRY_RELEASE.id` ב-**`5/5`**
  chunks **ישירות** ⇒ כל קומיט — כולל `docs` בלבד — מסובב `5/5` שמות
  (`0/1` CSS, שהוא הביקורת). ⚠️ הסיבה **ישירה ⇒ חזקה יותר**, ולכן ⛔ ניתן לשבור
  אותה ע"י שינוי גרף הייבוא.
- 🔴 **ומה שהאבחון של 13.09 מצא מעבר לזה: השומר של `B-164` היה מת בפרודקשן מיומו
  הראשון.** Vite פולט מחדש את תג ה-entry ומשמיט את ה-`id` שהשומר עוגן בו ⇒
  `getElementById('se-main-script')` החזיר `null` ⇒ `if (!el) return;`. נמדד
  ב-`dist/index.html`: **`1`** מופע של המחרוזת (בתוך `getElementById`) · **`0`**
  על התג. ⇒ `B-164` **מעולם ⛔ הגן**, וכל מדידה שהניחה שהוא מגן הניחה שקר.
  תוקן 13.09 ב-`B-320` — ראה `BACKLOG` ו-`probe:deploychunk`.

## #24 — 2026-09-17 — הסנטינל הכריז «משתמשים ⛔ יכולים למחוק עסקה», והכפתור ⛔ נלחץ מעולם

**התראת דיסקורד 20:36 UTC.** הארטיפקט `browser-findings` (ריצה `35271690173`,
`browser-findings-auth.json`, 908 בתים, ⛔ פג) נושא `2` ממצאים. השני, כלשונו:

```
"fp": "browser-auth|delete-failed",  "severity": "red",
"checked": "מחיקת עסקת הבדיקה דרך ה-UI (כולל דיאלוג האישור)",
"got":     "expected exactly 1 SNTNL1 row, found 0",
"reason":  "משתמשים לא יכולים למחוק עסקה — או שדיאלוג האישור נשבר",
"fix":     "בדוק את handleDeleteTrade ואת ConfirmProvider (src/components/ToastProvider.jsx)"
```

⚠️ **`SENTINEL_AUTH: 1` אומת** — לוג שורות `211` · `242`, ו-`schedule=[50 * * * *]
event=schedule auth=1` בשורה `206`. השכבה המחוברת **רצה.**

### מה ירה, ובאיזו שורה בדיוק

🔴 **`tests-sentinel/sentinel-auth.spec.js:437`**, בתוך `deleteRow`, שנקראה מ-**`:798`**
עם `RISK_TICKER` (כי `fixtureLive === true`), ונתפסה ב-**`:800-807`**.
⛔ **⛔ `:474`** — שם יושבת `toHaveCount(1)` שעברה · ⛔ **⛔ `:460`**, שנוסחה שונה
(`expected exactly 1 SNTNL row to rename`). המחרוזת מזהה שורה אחת ויחידה.

### שלוש ההשערות — שתיים **הופרכו במדידה**

- **(א) העסקה ⛔ נוצרה כלל — ⛔ הופרכה.** `create-failed` (`:747`) **⛔ בממצאים**,
  ולכן `:744` (`toHaveCount(1)`, timeout 20 שנ׳) עברה.
- **(ב) נוצרה, ה-rename נכשל — ⛔ הופרכה.** `fixture-failed` (`:768`) **⛔ בממצאים**.
  ⚠️ **וההוכחה החזקה יותר היא סדר הקריאות:** `logAnchorCounts` נקראת ב-**`:476`**,
  כלומר **אחרי** `:474` ו-`:475`; העובדה שהיא הדפיסה בכלל היא הראיה ששתיהן עברו:
  `sentinel anchor: SNTNL=0 SNTNL1=1 AAPL=1` · `sentinel control: legacy=0 anchor=1`.
- **(ג) נוצרה ושונתה, המחיקה נכשלה — אוששה בחלקה, ו*הנימוק שבממצא שגוי*.**

### 🔴 השורש: הכפתור ⛔ נלחץ, והדיאלוג ⛔ נפתח

`deleteRow` זורקת ב-**`:437`**, שהיא **שער הכניסה** — **לפני** `:444` (לחיצה על
`button[title="מחיקה"]`) ולפני `:445-447` (המתנה ל-`[role="dialog"]` ולחיצה על «מחק»).
⇒ **`handleDeleteTrade` ו-`ConfirmProvider` ⛔ הורצו על הריצה הזו כלל.**
⚠️ **שדות `reason` ו-`fix` שבממצא הם מחרוזות קבועות** שנכתבו מראש לענף הזה, והן
מצביעות בדיוק על שני הדברים ש⛔ בוצעו. **הממצא האשים את מה שלא רץ.**

⚠️ **והצלבת גיט מאשרת:** `src/components/ToastProvider.jsx` נגע לאחרונה ב-`819d169`
(**18.07**, לפני חודשיים) · `handleDeleteTrade` ב-`fadd61d` (**07.08**). ⇒ גלי 17.09
(`8d631ec` · `ec32e0e`) ⛔ נגעו במסלול המחיקה — **רגרסיה מהגלים שלנו הופרכה.**

### 🔴 השורה **הייתה** ב-DB, ו⛔ ב-DOM

`sentinel rest cleanup: removed 1 SNTNL/SNTNL1 row(s)` (`:533`). ⇒ ברגע הניקוי
השורה **קיימת בבסיס הנתונים**, בעוד `rowsFor(page,'SNTNL1').count()` החזיר **`0`**
שניות קודם לכן. ⛔ **זו ⛔ מחיקה שנכשלה — זו שורה שלא נמצאה על המסך.**

⚠️ **והאמבר ⛔ ירה, וזה נכון:** `:534` מותנה ב-`n > 0 && uiDeleteOk`, ו-`uiDeleteOk`
נותר `false` כי `deleteRow` זרקה ⇒ `ui-delete-incomplete` ⛔ הופק.

### ⛔ **אין שורה יתומה בפרודקשן**

`removed 1` ⇒ `N=1 > 0` ⇒ **נוקתה.** העוגן מדד `SNTNL=0 SNTNL1=1` ⇒ שורת בדיקה
**אחת** בדיוק, והיא זו שהוסרה. ⛔ **אין הצטברות** — `CLEANUP_FILTER` (`:487`) הוא
רשימה סגורה `ticker=in.(SNTNL,SNTNL1)` המוחקת את **שתיהן**. ⚠️ ריצת הסנטינל
המחוברת האחרונה היא `20:35`, ו⛔ רצה אחריה עוד אחת עד כתיבת שורות אלה.

### 🔴 מה שהופך את זה למרוץ ⛔ לרגרסיה: **אותו קומיט, תוצאה הפוכה**

| ריצה | קומיט | עוגן | `removed` | מחיקה |
|------|-------|------|-----------|--------|
| `35251831604` · 17:17 | `f420aa0` | `SNTNL=0 SNTNL1=1 AAPL=1` | **`0`** | ✅ עברה |
| `35271690173` · 20:35 | `f420aa0` | `SNTNL=0 SNTNL1=1 AAPL=1` | **`1`** | 🔴 נפלה |

**אותם בייטים · אותו מצב-מתקן · הפרש 3 שעות · תוצאה הפוכה** ⇒ **אי-דטרמיניזם.**
⚠️ **ו-18:17 (`35257908816`) ⛔ נספרת** — הארטיפקט שלה מכיל `browser-findings.json`
בלבד (`2` בתים, `[]`) ו⛔ קובץ `-auth` ⇒ השכבה המחוברת ⛔ רצה שם כלל.

### המנגנון החשוד — ⛔ אושש, ונרשם ככזה

**`:436` `await rows.count()` היא קריאת-השורות ה⛔-חוזרת ה*יחידה* במסע.** כל אחיותיה
חוזרות: `:744` (`toHaveCount`, 20 שנ׳) · `:474`/`:475` (15 שנ׳) · `:448` (15 שנ׳).
והיא יושבת מיד אחרי `openJournal` (`:783`), ש-**`:429` ממתינה בה ל-`JOURNAL_TABLE`
**הנראה** ⛔ לשורות שבתוכו** ⇒ טבלה שרונדרה ריקה לרגע מספקת את ההמתנה ומרעיבה את הספירה.

🔴 **⛔ אושש — והוא ⛔ ההסבר היחיד האפשרי.** הארטיפקט ⛔ יכול להבחין בין
«השורה ⛔ רונדרה מעולם» (**באג מוצר** — המשתמש ⛔ רואה את העסקה שלו אחרי מעבר טאב)
לבין «רונדרה רגע מאוחר יותר» (**מרוץ בבדיקה**). שתיהן עקביות עם כל מה שנמדד.
**המבחין הוא קריאה חוזרת שמדווחת את הפער**, והוא ⛔ בוצע: הרצה חוזרת של ה-workflow
נאסרה בגל הזה במפורש, כי הארטיפקט הוא הראיה.

### לקח

⚠️ **שער-כניסה ⛔-חוזר שיושב אחרי מעבר טאב מייצר האשמה אדומה נגד קוד ש⛔ הורץ.**
המחיר ⛔ «בדיקה מהבהבת» — הוא **כיוון חקירה שגוי**, חתום בשתי הפניות קונקרטיות
(`handleDeleteTrade` · `ConfirmProvider`) שנקראו כמדידה. ⛔ **הנימוח והתיקון שבממצא
חייבים לתאר את מה ש*נמדד*, ⛔ את מה שהענף נבנה כדי לחשוד בו.**

⛔ **אפס תיקון בגל הזה** — אבחון בלבד. הפריטים נרשמו ב-`BACKLOG`.

---

## #25 — 2026-09-18 — `| tail` בלע את קוד היציאה, ושער אדום נדחף ל-`main`

**קטגוריה:** CI / נוהל. ⛔ **אפס השפעה על משתמשים** — התוצר היחיד שנפגע הוא
המרשם עצמו.

### מה קרה

בגל `B-335`, אחרי שהקוד והמצבה כבר נחתו ירוקים, נמצאה סחיפה במספרי השורות
שנכתבו בפרוזה (`auth:535` — מספר ש⛔ קיים). התיקון נכתב, ונדחף בפקודה אחת:

```
npm run test:registry 2>&1 | tail -8 && git add … && git commit … && git push
```

🔴 **`| tail` מחזיר את קוד היציאה של `tail`, ⛔ של הסקריפט.** `test:registry`
הדפיס `❌ registry: 4/41 assertion(s) failed.` ויצא `1` — ו-`&&` **ראה `0`**,
כי זה מה ש-`tail` החזיר. הקומיט נוצר, ה-push עבר, ו-`47eaafc` נחת על `main`
כשארבע אסרציות אדומות.

### השורש בתוכן: `|` חשוף בתא טבלה

התיקון החליף מספרי שורות ב-fingerprints — ``auth|login-failed`` וכו׳. הצינור
בתוך ה-fingerprint הוא **תו הפרדת-עמודות של markdown** ⇒ כל אחד מחמשת
המזהים **שבר את התא לשניים**:

| אסרציה | לפני | אחרי |
|--------|------|------|
| 2.2 מקור+תאריך | 336/336 | **335/336** |
| 4.1 hash ב-DONE | 86/86 | **85/86** |
| 11.1 עמודת `T` | 86/86 | **85/86** |
| 13.2 שבעה תאים | 336/336 | **335/336** |

⚠️ **הכלל הזה כבר כתוב** — `CLAUDE.md` §14: «⛔ `|` בתוך תא טבלה במרשם —
`\|` תמיד», ושם מתועד שהשער תפס זאת **פעמיים בשבוע** (`M-018` · `C-038`,
04.09). זו הפעם ה**שלישית**, והראשונה שבה היא **הגיעה ל-`main`** — ⛔ כי
הכלל השתנה, אלא כי השער ⛔ **נשמע**.

### התיקון

`47eaafc..` — חמשת המזהים בשני הקבצים בורחו ל-`\|`; `test:registry` הורץ
**בלי צינור**, `41/41`, `EXIT=0`.

### לקח

🔴 **שער שפלטו עובר ב-pipe ⛔ שער — הוא הדפסה.** `cmd | tail && push` הוא
`push` ללא תנאי. ⛔ **`&&` אחרי צינור אינו מותנה בכלום**, וזה `R-4` בתחפושת
מכנית: לא ריככתי מספר כדי לעבור — ריככתי את **היכולת של השער לעצור**.

⚠️ **הכלל המעשי:** פקודה שמחליטה אם לדחוף מריצה את השער **חשוף**, או
`set -o pipefail`, או `; echo "EXIT=$?"` ונקראת בעיניים לפני ה-`git push`.
⛔ **`| tail`/`| head`/`| grep` על שער — רק כשה-push כבר קרה או ⛔ מתוכנן.**

⚠️ **ומה ש⛔ נשבר:** `pre-push` ⛔ עצר, כי הוא ⛔ מריץ את `verify` — הוא חוסם
`push` שאינו fast-forward (§5) ותו לא. ⛔ **ההגנה על תוכן המרשם היא `npm run
verify` ביד, ו⛔ hook** — כלומר היא **נוהל** ו⛔ מכניקה. נרשם ב-`BACKLOG`.
