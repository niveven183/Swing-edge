...
- **אל תסמוך על `rows` מ-Supabase MCP `list_tables` כמדד ספירה.** הערך הוא
  `pg_class.reltuples` — הערכת planner, לא ספירה. ב-27.07 הוא הראה 287 בעוד
  `count(*)` החזיר 285, ותוקן רק ב-autovacuum של 18:56:34. **כל מספר שנכנס לדוח
  חייב לבוא מ-`count(*)`.** הפער הזה נראה כמו "מדידה לא דטרמיניסטית" והוא לא —
  שני נתיבי קריאה שונים. השוואת psql מול MCP טרם בוצעה: psql אינו מותקן מקומית
  ו-`SUPABASE_DB_URL` הוא סוד Actions בלבד (§12), ולכן היא מתבצעת בריצת ה-dispatch
  הראשונה של `user-analytics.yml` (שלוש ספירות `count(*)` רצופות)
- **173 עסקאות נמחקו ב-27.07 (287 → 114) ואין דרך לדעת זאת בדיעבד.** אין
  `deleted_at`, אין `updatedAt` — מחיקה שקופה לחלוטין לכל שאילתת חלון-זמן. זו הסיבה
  ש-`user-analytics.yml` שומר ספירות מצטברות ב-Actions cache: בלי בסיס השוואה
  מתמשך, אובדן נתונים בקנה מידה כזה אינו מייצר שום אות
