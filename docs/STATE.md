...
**תיקוני שלמות מספרית לפי AUDIT-2026-07-27 — קבוצה 1** (שלמות שרשרת הנתונים:
`is_demo` שלא נקרא בחזרה · `exitDate` שאינו ב-`TRADE_COLUMNS` · `CLOSED` בלי `exit`).
הקבוצה היחידה שמשחיתה נתונים באופן פעיל — כל יום פתוח מוסיף עסקאות דמו מזוהמות.
...
- **`is_demo` נכתב ל-Supabase ומעולם לא נקרא בחזרה** (FIN-039). עסקאות דמו הופכות
  לאמיתיות אחרי reload והדגל נדרס ל-`false` בשמירה הבאה — **השחתה פעילה ובלתי-הפיכה**
  שממשיכה להצטבר כל עוד הממצא פתוח. זו הסיבה שקבוצה 1 היא הראשונה
