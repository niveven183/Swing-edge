...
### 0.6 🔴 פריט 6 — ההנחה שלו הופרכה ע"י אודיט שנדחף אמש

**זו הסטייה החשובה מבין השש.**

`b221e33` (31.07, 23:31) — `docs/audits/AUDIT-2026-08-01-ocr-diagnosis.md`.
נדחף אחרי שנוסח הפרומפט, ומפריך את הנימוק שבו.

הפרומפט מנמק את פריט 6: *"עדות משתמש 31.07: היה תופס בעצמו את 'מחיר נוכחי
בתור כניסה'"*. האודיט קובע (§0, §2א) שהתסמין הזה **אינו OCR כלל**:

> "ה-OCR החזיר `entry: null` … **את המחיר הנוכחי כתב לשדה `entry` אפקט הציטוט
> החי בצד הלקוח, כ-250ms אחרי שה-OCR סיים.**"

הרצף הדטרמיניסטי (`AUDIT §2א`):
1. `App:2460` — הטיקר מה-OCR נכתב לטופס
2. `App:2461-2463` — entry/stop/target נשארים ריקים (`result.entry === null`)
3. `App:1930-1937` — `useEffect` תלוי ב-`analyzerForm.ticker` נורה, debounce 250ms
4. `App:1922` — `if (!force && f.entry) return f;` → `entry` ריק → **ממשיך**
5. `App:1923` — המחיר החי נכתב ל-Entry

> "**השומר בשורה 1922 מדלג רק כשה-entry כבר מלא. כלומר המילוי-האוטומטי יורה
> בדיוק ורק כשה-OCR נכשל.**"

**ההשלכה הקטלנית לפריט 6 כפי שנוסח:** מסך אישור **לבדו אינו מתקן דבר.**
הזרימה תהיה — כרטיס אישור מציג "Entry: לא זוהה" → המשתמש מאשר → הטיקר נכתב
לטופס → **האפקט נורה ומוסיף את המחיר הנוכחי ל-Entry, אחרי האישור.**
המשתמש יקבל בדיוק את אותו ערך שגוי, והפעם אחרי שראה במו עיניו שהשדה ריק
ו"אישר". **מסך אישור בלי F1 אינו הגנה — הוא תפאורה.**

האודיט מדרג את מסך האישור עצמו כ-"אחרי מדידת ה-30", ומציין:
> "F1–F3 מקטינים את דחיפותו אך **אינם מחליפים** אותו"

**המסקנה: פריט 6 חייב לכלול את F1, אחרת אין לו ערך.** לשמחתנו F1 ו-F3
שניהם **צד לקוח בלבד** ואינם נוגעים ב-`api/ocr.js` — כלומר נכנסים בתוך
האילוץ שהצבת. F2 יושב ב-`api/ocr.js` ולכן **נשאר בחוץ**.

| חוסם | מיקום | בהיקף? |
|---|---|---|
| **F1** — לא למלא Entry ממחיר חי כשמקור המילוי הוא העלאת תמונה | `App:1920-1924`, `:1930-1937` (אנלייזר) · `:1891-1895`, `:1901-1908` (Log New Trade) | ✅ צד לקוח |
| **F3** — לגזור את הבאדג' ממה שבאמת התמלא, לא מ-`confidence` בלבד | `App:4116-4130` | ✅ צד לקוח |
| F2 — לא להעביר `modelConf` כשאין כלי פוזיציה | `api/ocr.js:422` | ⛔ מחוץ להיקף |

**דורש את הכרעתך** — שלוש אפשרויות ב-§2 פריט 6.
...
> ⚠️ **קרא את §0.6 לפני שתאשר את הפריט הזה.** מסך אישור **לבדו** אינו מתקן
> את מה שעומרי חווה — אפקט הציטוט החי ימלא את Entry אחרי האישור. שלוש אפשרויות:

| # | היקף | תוצאה |
|---|---|---|
| **6-A** | מסך אישור בלבד (כפי שנוסח) | ⛔ **לא מומלץ.** הבאג שורד; המשתמש "מאשר" ואז השדה מתמלא לבדו |
| **6-B** ✅ | **F1 + מסך אישור** | מומלץ. סיבת השורש נסגרת, והאישור הופך את מה שנשאר לגלוי |
| **6-C** | F1 + F3 + מסך אישור | הכי שלם. מוסיף את הבאדג' הכוזב (✓ ירוק על טופס ריק). מגדיל את הדיף |

**ברירת המחדל שאבצע אם לא תאמר אחרת: 6-B.** F3 הוא באג נפרד ומוכר (רשום
כ-F3 באודיט), ולכן מותר לו לחכות; F1 אינו יכול — בלעדיו פריט 6 אינו סוגר כלום.

**F1 — התיקון:** לשומר ב-`:1922` יש תנאי אחד (`f.entry` ריק). נוסף לו מקור:
מילוי-אוטומטי ממחיר חי מדלג גם כשהטיקר הגיע מ-OCR ולא מהקלדה. דגל ref
(`tickerFromOcrRef`) נקבע ב-`handleAnalyzerImageUpload` לפני כתיבת הטיקר,
ומנוקה כשהמשתמש נוגע בשדה הטיקר ידנית. אותו טיפול ב-Log New Trade
(`:1891-1895`, `:1901-1908`) — האודיט מציין במפורש שהבאג הועתק לשני המסכים.
`force = true` (רענון יזום בלחיצה) ממשיך לעקוף — זו בקשה מפורשת של המשתמש.
...
| **F1 — צילום בלי כלי פוזיציה** (תרחיש עומרי) | אחרי "החל": טיקר מלא, **Entry נשאר ריק**. זו הבדיקה שמפרידה 6-B מ-6-A |
| F1 — הקלדת טיקר ידנית, בלי צילום | המחיר החי **כן** ממלא Entry — ההתנהגות הקיימת נשמרת |
| F1 — אותו תרחיש ב-Log New Trade | זהה. האודיט מציין שהבאג בשני המסכים |
...
**F2 נשאר חוסם למדידת ה-30** (`api/ocr.js` מחוץ להיקף) · F3 אם נבחר 6-B ולא 6-C
**פריט 13 באודיט/STATE:** אם 6-B מאושר, F1 נסגר — לעדכן את דירוג החוסמים
...
- ⛔ `api/ocr.js` — לפי הוראה מפורשת. **גורר ש-F2 (`:422`, `modelConf` כוזב
  כשאין כלי פוזיציה) נשאר פתוח** — נשאר חוסם למדידת ה-30, שורת ⏭️ ב-`STATE.md`
- ⛔ החזרת `null` מ-`profitFactorFromPnls` — מוקש `isFinite(null)`, מתועד
- ⛔ פריט 4ב ללא אישור נפרד
- ⛔ פריט 6 בגרסת 6-A (מסך אישור בלי F1) — תפאורה, ראה §0.6
