...
check("11.2", `${done.filter((d) => /T3/.test(d.cells[T_COL] ?? "") && !RETRO.test(d.cells[T_COL] ?? "")).length} שורות T3 לא-רטרו · כולן נושאות אימות עין/מקור`,
  blindT3.length === 0,
  blindT3.map((d) => `${d.id} (${d.file}:${d.lineNo}) — T3 בלי סמן אימות-עין/מקור. ⛔ אסרציה ירוקה אינה מסך`).join("\n      "));
