...
export const makeConvertingCalc = (table, displayCurrency, status) =>
  (trade) => {
    ...
    //
    // ── `fxUnconverted` — הסימון (B-142, 2026-08-15) ────────────────────────
    //
    // 🔴 עד היום הסירוב חזר **חסר-שם**: הקורא קיבל אובייקט מטריקות תקין
    // לחלוטין שערכו נקוב במטבע **אחר** מזה שהכותרת מדפיסה, ⛔ ובלי שום דרך
    // להבדיל בינו לבין הצלחה. ⇒ `capital + Σ pnl` חיבר יחידות שונות, ו⛔שום
    // סמל לא הפך את זה לנכון. זה היה `B-142`.
    //
    // ⚠️ הסימון הוא **תוצר-לוואי של `d`** — הכרעה שכבר התקבלה בשורה שמעל.
    // ⛔ לא מעבר שני ו⛔לא חישוב מקביל: שני מעברים על אותה שאלה נסחפים זה מזה.
    //
    // ⛔ **⛔ אינו מתייג מטבע מחדש ו⛔אינו משנה ערך.** `identity` (`d.ok === true`)
    // מחזיר את **אותו** `m` בדיוק — 43/46 המשתמשים ⛔ אינם רואים ביט שזז,
    // ו-13.9b‴ נשארת ירוקה. מסלול הסירוב מחזיר את כל השדות הקיימים כפי שהם
    // ומוסיף שדה **חדש** בלבד; הצרכן (`closedPnL` · `generateEquityCurve`)
    // מוציא את האיבר מהסכום ו**סופר** אותו. ⛔ לא מנחש, ⛔ ולא משמיט בשקט.
